...
Intent check found that some unsupported facts were missed because retrieval queries did not match
...
- Misaligned facts (JSON keyed by fact1, fact2, ...)
...
1. Return `fact_queries` as an array only for the provided misaligned fact keys and no extra keys.
2. Each object must include `fact_key`, `fact`, and `search_queries`.
3. For each object, echo the misaligned fact text exactly in `fact`.
